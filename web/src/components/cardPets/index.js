...
import PopUpAdopt from "../PopUpToAdopt";

// import photoPet1 from "../../img/gato.png"

const CardPets = ({ dataPet }) => {
  const [buttonPopup, setButtonPopup] = useState(false);
  const [buttonPopupAdopt, setButtonPopupAdopt] = useState(false);
  return (
    <C.Container>
      ...
      <Button
        ...
        onClick={() => setButtonPopupAdopt(true)}
        key={dataPet.id}
      >
        Adotar
      </Button>
      ...
      <PopUpAdopt
        dataprops={dataPet}
        trigger={buttonPopupAdopt}
        setTrigger={setButtonPopupAdopt}
      />
    </C.Container>
  );
};
